...
using namespace std;
                 ...
                 of the boundary conditions). This class is derived from the
                 ParameterHandler class in <b>deal.II</b>. */
template <int dim>
  ...
  public ParameterHandler
{
  public:
    ...
    map<unsigned char, const Function<dim> *> boundary_map;
                 ...
                 /** Maps of boundary value functions for homogeneous Dirichlet
                 boundary values:
                 1st: a boundary indicator;
                 2nd: a zero boundary value function. */
    map<unsigned char, const Function<dim> *> zero_boundary_map;

                 /** Vector of flags for distinguishing between velocity and
    ...
    map<unsigned int, double> boundary_values;
                 ...
                 /** Flag to indicate whether or not the Newton iteration scheme
                 must update the Jacobian at each iteration. */
    bool update_jacobian_continuously;

                 /** Flag to indicate whether or not the time integration
                 scheme must be semi-implicit. */
    bool semi_implicit;

                 /** Flag to indicate how to deal with the non-uniqueness of
                 the pressure field. */
    bool fix_pressure;

                 /** Flag to indicate whether homogeneous Dirichlet boundary
   ...
   enum MaterialModel {INH_0 = 1, INH_1, CircumferentialFiberModel};
                 ...
                 /** Variable to identify the constitutive model for the
                 immersed solid. */
   unsigned int material_model;

                 /** String to store the name of the finite element that will
                 be used for the pressure field. */
  std::string fe_p_name;
};

                 /** Class constructor: the name of the input file is
                 <i>immersed_fem.prm</i>. If the file does not exist at run
                 time, it is created, and the simulation parameters are given
                 default values. */
template <int dim>
                 ...
                 component_mask(dim+1, true)
{
  ...
  this->leave_subsection();
                 ...
                 // Declaration of the class parameters and assignment of
                 // default values.
  this->declare_entry (
  ...
  if(this->get("Solid constitutive model") == std::string("INH_0"))
     ...
     material_model = INH_1;
                 ...
                 == std::string("CircumferentialFiberModel"))
  ...
  fe_p_name +="<dim>(" + Utilities::int_to_string(degree-1) + ")";
   ...
   this->print_parameters(paramfile, this->Text);
                 ...
                 multiplier for enforcing incompressibility both in the fluid
                 and in the immersed domains for a specific problem. The latter
                 concerns the equilibrium of a circular cylinder immersed in an
                 incompressible Newtonian fluid.  The immersed domain is assumed
                 to be an incompressible elastic material with an elastic
                 response proportional to the stretch of elastic fibers wound in
                 the circumferential direction (hoop). The constitutive
                 equations of the cylinder correspond to the choice of
                 "Solid constitutive model" as "CircumferentialFiberModel".
                 Finally, we refer to this particular problem as the
                 "Hello world" problem for immersed methods.  We learned this
                 expression from our colleague Boyce E. Griffith, currently at
                 the Leon H. Charney Division of Cardiology, Department of
                 Medicine, NYU School of Medicine, New York University. */
template<int dim>
class ExactSolutionRingWithFibers :
  public Function<dim>
{
  public:
                 /** No default constructor is defined. Simulation objects must
                 be initialized by assigning the simulation parameters, which
                 are elements of objects of type <b>ProblemParameters</b>. */
                                 ...
                                 ) const
{
  ...
  for (unsigned int i = 0; i < values.size(); ++i)
    vector_value (points[i], values[i]);
                 ...
                 /** It defines simulations objects. The only method in the
                 public interface is <b>run()</b>, which is invoked to carry
                 out the simulation. */
template <int dim>
class ImmersedFEM
{
  public:
                 ...
                 /** No default constructor is defined. Simulation objects must
                 be initialized by assigning the simulation parameters, which
                 are elements of objects of type ProblemParameters. */
  ...
  private:
    ...
    ProblemParameters<dim> &par;
                 ...
                 /** Vector of boundary indicators. The type of this vector
                 matches the return type of the function
                 <b>Triangulation< dim,
                 spacedim >::get_boundary_indicator()</b>. */
    std::vector<unsigned char> boundary_indicators;

                 /** Triangulation over the control volume (fluid domain).
                 Following <b>deal.II</b> conventions, a triangulation pertains
                 to a manifold of dimension <i>dim</i> embedded in a space of
                 dimension <i>spacedim</i>. In this case, only a single
                 dimensional parameter is specified so that the dimension of the
                 manifold and of the containing space are the same. */
    Triangulation<dim> tria_f;

                 /** Triangulations of the immersed domain (solid domain).
                 Following <b>deal.II</b> conventions, a triangulation pertains
                 to a manifold of dimension <i>dim</i> embedded in a space of
                 dimension <i>spacedim</i>. While in this case the two dimension
                 parameters are set equal to each other, it is possible to
                 formulate problems in which the immersed domain is a manifold
                 of dimension lower than that of the containing space. */
    Triangulation<dim, dim> tria_s;

                 /** FESystems for the control volume. It consists of two
                 fields: velocity (a vector field of dimension <i>dim</i>) and
                 pressure (a scalar field). The meaning of the parameter
                 <i>dim</i> is as for the <b>Triangulation<dim> tria_f</b>
                 element of the class. */
    FESystem<dim> fe_f;

                  /** A variable to check whether the pressure field is
                  approximated using the FE_DGP elements. */
    bool dgp_for_p;

                 /** This is FESystems for the immersed domain. It consists of a
                 single field: displacement (a vector field of dimension
                 <i>dim</i>). The meaning of the parameter <i>dim</i> is as for
                 the <b>Triangulation<dim> tria_s</b> element of the class. */
    ...
    DoFHandler<dim, dim> dh_s;
                 ...
                 /** The triangulation of for the immersed domain defines the
                 reference configuration of the immersed domain. As the immersed
                 domain moves through the fluid, it is important to be able to
                 conveniently describe quantities defined over the immersed
                 domain according to an Eulerian view. It is therefore
                 convenient to define a <b>MappingQEulerian</b> object that will
    ...
    BlockSparseMatrix<double> dummy_JF;
                 ...
                 /** State of the system at current time step: velocity,
                 pressure, and displacement of the immersed domain. */
    BlockVector<double> current_xi;

                 /** State of the system at previous time step: velocity,
                 pressure, and displacement of the immersed domain. */
    ...
    BlockVector<double> newton_update;
                 ...
                 /** Vector to compute the average pressure when the average
    ...
    Vector<double> unit_pressure;
                     ...
                     /**A vector that can be used as a temporary storage of the
                     data in current_xi.  This object is defined simply for
                     convenience.  It is defined as a private member of the
                     class to avoid that the object is allocated and deallocated
                     when used, so to gain in efficiency. */
    BlockVector<double> utility_xi;

                 /** Number of degrees of freedom for each component of the
                 system. */
    ...
    unsigned int constraining_dof;
                 ...
                 /** A container to store the dofs corresponding to the pressure
                 field. */
    set<unsigned int> pressure_dofs;

                 /** Storage for the elasticity operator of the immersed
                 domain. */
    ...
    double area;
                 ...
                 /** Filestream that is used to output a file containing
                 information about the fluid flux, area and the centroid of the
                 immersed domain over time. */
        ...
        quad_s (qtrapez, 4*(par.degree+8))
{
  ...
  global_info_file.open((par.output_name+"_global.gpl").c_str());
                 ...
                 /** Distructor: deletion of pointers created with <b>new</b>
                 and closing of the record keeping file. */
template <int dim>
  ...
  if(vec.size() != 0)
    ...
    for(;it != itend; ++it)
      constraints_f.set_inhomogeneity(it->first, it->second);
                 ...
                 the immersed domain.  It distributes degrees of freedom over
                 said triangulations. Both grids are assumed to be available in
                 UCD format. The naming convention is as follows:
                 <i>fluid_[dim]d.inp</i> for the control volume and
                 <i>solid_[dim]d.inp</i> for the immersed domain. This function
                 also sets up the constraint matrices for the enforcement of
                 Dirichlet boundary conditions. In addition, it sets up the
                 framework for enforcing the initial conditions. */
template <int dim>
void
ImmersedFEM<dim>::create_triangulation_and_dofs ()
{
  ...
  all_dofs[1] = n_dofs_W;
                 ...
                 // Re-initialization of the BlockVectors containing the values
                 // of the degrees of freedom and of the residual.
                 ...
                 // We now deal with contraint matrices.
  {
    ...
    DoFTools::make_hanging_node_constraints (dh_s, constraints_s);
                 ...
                 // To solve the problem we first assemble the Jacobian of the
                 // residual using zero boundary values for the velocity. The
                 // specification of the actual boundary values is done later by
                 // the "apply_current_bc" function.
     VectorTools::interpolate_boundary_values (MappingQ1<dim>(),
                 ...
                 // Determine the area (in 2D) of the control volume and find
                 // the first dof pertaining to the pressure.
  ...
  constraints_s.close ();
                 ...
                 // defined here only in the eventuality that the
                 // VectorTools::project function is used in initializing
                 // the vectors previous_xi.block(0) and unit_pressure.
  ConstraintMatrix cc;
  cc.close();
                 ...
                 // We now deal with the sparsity patterns.
  {
    ...
    csp.block(1,1).reinit (n_dofs_W , n_dofs_W );
                 ...
                 // As stated in the documentation, now we *must* call the
                 // function csp.collect_sizes.() since have changed the size of
                 // the sub-objects of the object csp.
    csp.collect_sizes();
    ...
    for(unsigned int i=0; i<dim; ++i)
    {
      ...
      for(unsigned int j=0; j<dim; ++j)
                 // The velocity components are coupled with themselves and each
                 // other.
    ...
    coupling(dim, dim) = DoFTools::always;
                 ...
                 // Find the first pressure dof.  Then tell all the pressure
                 // dofs that they are related to the first pressure dof.
  ...
  apply_current_bc(previous_xi, 0);
                 ...
                 // Resizing other containers concerning the elastic response of
                 // the immersed domain.
                 ...
                 )
{
  ...
  std::vector< std::vector< Tensor<1,dim> > > local_grad_up(
                 nqpf,
  ...
  for(; cell != endc; ++cell)
  {
    ...
    fe_f_v.reinit(cell);
                   ...
                   // Values of the partial derivative of the velocity relative to
                   // time at the quadrature points on the current fluid cell.
                   // Strictly speaking, this vector also includes values of the
                   // partial derivative of the pressure with respect to time.
    fe_f_v.get_function_values(xit.block(0), local_upt);

                   // Values of the velocity at the quadrature points on the
                   // current fluid cell. Strictly speaking, this vector also
                   // includes values of pressure.
    fe_f_v.get_function_values(xi.block(0), local_up);

                   // Values of the gradient of the velocity at the quadrature
                   // points of the current fluid cell.
    fe_f_v.get_function_gradients(xi.block(0), local_grad_up);

                   // Values of the body force at the quadrature points of the
                   // current fluid cell.
...
ImmersedFEM<dim>::run ()
{
                 // Initialization of the time step counter and of the time
                 // variable.
  ...
  bool update_Jacobian = true;
                 ...
                 // While this code employs a "home-made" implicit Euler time
                 // integration scheme, we set things up so to facilitate the
                 // use of other time integrators, e.g., DAE. Hence, we define a
                 // Vector<double> object that contains the same information as
                 // the previous_xi, but that can be passed to integrators that
                 // do not accept objects of type BlockVector.
  ...
  {
                 // Initialization of two counters for monitoring the progress
    ...
    unsigned int outer_nonlin_iter = 0;
      ...
      while(true)
      {
                 ...
                 // Denoting the current time step by n, the vector xi'(t_{n})
                 // is assumed to be a linear combination of xi(t_{i}), with
                 // i = n - m ... n, with m <= n. For simplicity, here we
                 // implement an implicit Euler method, according to which
                 // xi'(t_{n}) = [xi(t_{n}) - xi(t_{n-1})]/dt, where dt is the
        ...
        {
                 // If the norm of the residual is not sufficiently small, make
                 // a note of it and compute an update.
          std::cout
            << nonlin_iter
            << ": "
            << res_norm
            << std::endl;
                 ...
                 // To compute the update to the current xi, we first change the
                 // sign of the  current value of the residual ...
         current_res *= -1;
                 // ... then we compute the update, which is returned by the
                 // method "solve" of the object "JF_inv". The latter is of
                 // class "SparseDirectUMFPACK" and therefore the value of the
                 // (negative) of the current residual must be supplied in a
                 // container of type Vector<double>.  So, we first transfer the
                 // information in "current_res" into a Vector<double> called
                 // "tmp" and then we carry out the computation of the update.
         static Vector<double> tmp(current_res.size());
         tmp = current_res;
         JF_inv.solve(tmp);

                 // Now that we have the updated of the solution into an object
                 // of type Vector<double>, we repackage it into an object of
                 // type BlockVector.
         newton_update = tmp;
         ...
         current_xi.add(1., newton_update);
                 ...
                 // We are here because the solution needed to be updated. The
                 // update was computed using whatever Jacobian was available.
                 // If, on entering this section of the loop, the value of the
                 // residual was very poor and if the solution's method
                 // indicated in the parameter file did not call for a
                 // continuous update of the Jacobian, now we make sure that the
                 // Jacobian is updated before computing the next solution
                 // update.
          if(res_norm > 1e-2) update_Jacobian = true;
                 ...
                 // convergence is not achieved in 15 iterations update the
                 // Jacobian and try again.  The maximum number of 15-iteration
                 // cycles is set (arbitrarily) to three. The counter for the
                 // cycle is "outer_nonlin_iter".
        ++nonlin_iter;
        if(nonlin_iter == 15)
        {
          ...
          std::printf(
                 ...
                 // If convergence is not in our destiny, accept defeat, with as
                 // much grace as it can be mustered, and go home.
        AssertThrow (outer_nonlin_iter <= 3,
                 ExcMessage ("No convergence in nonlinear solver."));
      }

                 // We have computed a new solution.  So, we update the state of
                 // the system and move to the next time step.
                 ...
                 )
{
  ...
  }
  {
                 ...
                 // Compute area of the solid, and location of its center of
                 // mass.
    typename DoFHandler<dim,dim>::active_cell_iterator
                 cell = dh_s.begin_active(),
                 endc = dh_s.end();
                 ...
                 )
{
  ...
  std::vector< std::vector< Tensor<1,dim> > >
                 H(qsize, std::vector< Tensor<1,dim> >(dim));
                 ...
                 )
{
  ...
  bool update_vecF = (vec_F.size()!= 0);
                 ...
                 // CircumferentialFiberModel is used.
   Point<dim> p;
                 ...
                 )
{
  for (unsigned int i = 0; i < dofs.size(); ++i)
    local_M_gamma3_inv_A_gamma (i) = M_gamma3_inv_A_gamma(dofs[i]);
                 ...
                 /** Calculate the error for the equilibrium solution of
                 corresponding to a ring with circumferential fibers. */
template <int dim>
void
