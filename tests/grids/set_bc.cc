...
main()
{
  ...
  {
    GridIn<3,3> gi;
    gi.attach_triangulation(tria_isotropic);
  ...
  for(int i=0; i<2; ++i) 
	...
	{
	  cell = tria_isotropic.begin_active();
	  endc = tria_isotropic.end();
	}
      for(; cell != endc; ++cell) {
	for(unsigned int f=0; f < GeometryInfo<3>::faces_per_cell; ++f)
	  if(cell->face(f)->at_boundary()) {
	    // Inflow
	    if(abs(cell->face(f)->center()[0])<1e-10) {// x=0 
	      cell->face(f)->set_boundary_indicator(1); // 1 = inflow
	      deallog << "Inflow: " << cell->face(f) 
		      << ", face center: " << cell->face(f)->center() 
		      << std::endl;
	    }
	    else if(abs(cell->face(f)->center()[0] - 25.0)<1e-9) {
  ...
  ofile.close();
  ofile.open(SOURCE_DIR "/../../meshes/SchaeferTurek_3d_isotropic_id.msh");
